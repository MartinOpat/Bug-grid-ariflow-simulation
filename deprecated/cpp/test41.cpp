...
// Grid parameters
const int X_SIZE = 400;  // Should be divisible by 2
const int Y_SIZE = 200;
const int Z_SIZE = 200;
...
void adaptive_runge_kutta(std::vector<std::vector<std::vector<std::vector<double>>>>& u, double& t, double& dt,
    ...
    while (!step_accepted) {
        ...
        #pragma omp parallel for collapse(4) reduction(max:max_error)
        for (int comp = 0; comp < 3; ++comp)
            for (int i = 0; i < nx; ++i)
                for (int j = 0; j < ny; ++j)
                    for (int k = 0; k < nz; ++k) {
                        ...
                        max_error = std::max(max_error, norm_err);
                    }
