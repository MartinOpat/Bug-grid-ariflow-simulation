...
class CustomPDE(pde.PDEBase):
    ...
    #     @jit(nopython=True, parallel=True)
    #     def clamp_vector_field(vector_field, val):
    #         result = np.zeros_like(vector_field, dtype=np.float64)
    #         for x in prange(X_SIZE):
    #             for y in range(Y_SIZE):
    #                 for z in range(Z_SIZE):
    #                     result[0][x][y][z] = min(max(vector_field[0][x][y][z], -val), val)
    #                     result[1][x][y][z] = min(max(vector_field[1][x][y][z], -val), val)
    #                     result[2][x][y][z] = min(max(vector_field[2][x][y][z], -val), val)
    #         return result
