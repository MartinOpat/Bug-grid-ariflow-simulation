...
from pde.solvers import ExplicitMPISolver, Controller
import time
...
###########################################################
solver_mpi = ExplicitMPISolver(eq, scheme = 'rk45', decomposition=2, backend = 'auto', adaptive = True)
controller_mpi = Controller(solver_mpi, t_range = 1)
# state = eq.get_initial_state(grid)
#state = eq.get_initial_state(grid, storage, Bini, Wini, Hini)      
result = controller_mpi.run(field)
